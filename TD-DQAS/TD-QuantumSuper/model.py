...
#
# Rs_space = list(itertools.product(valid_Rs, valid_Rs, valid_Rs, valid_Rs))  # due to qubit
...
def circuit_search(params, wires=[0,1,2,3],hamiltonian=None, n_qubits=3, n_layers=3, arch=[]):
    '''
    quantum circuit
    '''
    # nas circuit
...
class CircuitSearchModel():
    def __init__(self, dev, NAS_search_space, Rs_space,CNOTs_space,n_qubits=3, n_layers=3, n_experts=5):
        self.dev = dev
        self.n_qubits = n_qubits
        self.n_layers = n_layers
        ...
        self.Rs_space = Rs_space
        self.CNOTs_space = CNOTs_space
    ...
    def get_params(self, subnet, expert_idx):
        ...
        for j in range(self.n_layers):
            r_idx = subnet[j] // len(self.CNOTs_space)
            params.append(self.params_space[expert_idx, j, r_idx:r_idx+1])  # 1*4
        return np.concatenate(params, axis=0)

    def set_params(self, params):
        for j in range(self.n_layers):
            r_idx = self.subnet[j] // len(self.CNOTs_space)
            self.params_space[self.expert_idx, j, r_idx:r_idx+1] = params[j, :]

    def load_params(self,params):

        self.params_space = params

    def __call__(self, params, wires,hamiltonian,n_qubits,n_layers,subnet):
        loss = circuit_search(params, wires=wires,hamiltonian=hamiltonian,
                       n_qubits=n_qubits,
                       n_layers=n_layers,
                       arch=subnet)
    ...
    def cost(self, params, wires,hamiltonian,n_qubits,n_layers,subnet):
        loss = circuit_search(params, wires=wires,hamiltonian=hamiltonian,
                       n_qubits=n_qubits,
                       n_layers=n_layers,
                       arch=subnet)
...
class CircuitSearchModel_FTSO():
    def __init__(self, dev, NAS_search_space, Rs_space,CNOTs_space,n_qubits=3, n_layers=3, n_experts=5):
        ...
        '''init params'''
        # randomly initialize parameters from a normal distribution
        self.params_space = np.random.uniform(0, np.pi * 2, (n_experts,n_layers,n_qubits))
    ...
    def get_params(self,idx):
        self.params = self.params_space[idx]
        return self.params

    def set_params(self, params,idx):
        self.params_space[idx] = params
